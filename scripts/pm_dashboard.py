...
# Vivid palette — readable over daytime camera footage
_BG  = "#0A0C12"      # solid background (opaque mode)
_DIV = "#3A3F58"      # panel dividers
_WHT = "#FFFFFF"      # primary text
_DIM = "#B0BAD4"      # secondary labels (bright enough to read in sunlight)
...
def render_frame(W, H, pt, speed_kmh, odo_km, weather, fonts,
                 units="imperial", transparent=False):
    """
    Render one dashboard frame.
    Layout: [Compass | Speed + Odometer | Weather]
    Speed panel: primary unit (large) → label → secondary unit (large) → label → odometer.
    All draw.text calls use anchor="lt" so y always means top-of-ink, matching _twh.
    """
    if transparent:
        img  = Image.new("RGBA", (W, H), (0, 0, 0, 0))
        # Semi-transparent dark panel behind all instruments
    ...
    CCX = PW // 2            # compass centre x
    CCY = H // 2 - 10        # compass centre y (slightly above midpoint)
    CR  = min(PW, H) // 3    # ring radius
    ...
    for deg in range(0, 360, 10):
        major = (deg % 30 == 0)
        a     = math.radians(deg - 90)          # 0° compass = up = −90° math angle
        r_out = CR
    ...
    # Heading needle — white tip points to current heading; red tail is opposite end
    heading = pt.get("heading", 0.0)
    ang     = math.radians(heading - 90)     # compass → screen angle
    NL = int(CR * 0.75)
    TL = int(CR * 0.28)
    nw = max(3, int(CR * 0.07))              # needle width scales with compass size
    ...
    # Primary/secondary order follows --units flag
    if units == "imperial":
    ...
    # Measure every stack element before placing anything
    ...
    # Pin odometer proportionally from the bottom; centre speed block above it
    HDR_BOT = max(20, int(H * 0.06))
    ODO_PAD = max(10, int(H * 0.07))
    odo_y   = H - ODO_PAD - oh
    GA = max(4,  int(psh * 0.08))   # number → its label
    GB = max(12, int(psh * 0.22))   # primary group → secondary number
    GC = max(4,  int(ssh * 0.08))   # secondary number → its label
