#!/usr/bin/env python3
"""
clops_dashboard.py — CamClops instrument dashboard renderer
...
Usage:
    clops_dashboard.py --manifest <path> --trip <id> --output <path.mp4>
...
def build_points(gps_track, gps_lock_offset=0):
    ...
    gps_lock_offset aligns T=0 with video start (same convention as clops_maprender.py).
    """
    if not gps_track:
        return []
...
def compute_derived_headings(points, min_move_m=2.0):
    """Position-derived bearing replaces GPS track heading. See clops_hud.py."""
    if len(points) < 2:
        return points
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CamClops instrument dashboard renderer — GPS track → MP4 / WebM"
    )
    parser.add_argument("--manifest",    required=True,
                        help="CamClops manifest JSON path")
    ...
    _brand_output(args.output, "dashboard", args.ffmpeg)
