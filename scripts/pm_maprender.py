#!/usr/bin/env python3
"""
pm_maprender.py — PathMux moving map renderer

Reads a GPS track from a PathMux manifest JSON and renders a north-up,
...
Tiles are fetched from OpenStreetMap and cached in ~/.cache/pathmux/tiles so
...
Usage:
    pm_maprender.py --manifest <path> --trip <id> --output <path.mp4>
                    [--width 960] [--height 540] [--fps 30]
                    [--render-fps 10] [--trail-seconds 60]
                    [--ffmpeg ffmpeg] [--cache-dir ~/.cache/pathmux/tiles]
...
def fetch_tile(tx, ty, zoom, cache_dir, session):
    ...
    headers = {
        "User-Agent": "PathMux/1.2 dashcam-gps-renderer (https://github.com/Nutball-Labs/PathMux)"
    }
    try:
...
def main():
    parser = argparse.ArgumentParser(
        description="PathMux moving map renderer — GPS track → MP4"
    )
    parser.add_argument("--manifest",      required=True,  help="Path to PathMux manifest JSON")
    ...
    cache_dir = args.cache_dir or os.path.join(
        os.path.expanduser("~"), ".cache", "pathmux", "tiles"
    )
