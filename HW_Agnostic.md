...
The following files in `~/.config/pathmux/profiles/` use stale formats and will need
to be regenerated by running `pm_probe --wizard` after the new code is in place:
...
## pm_probe Wizard Updates (tools/pm_probe.cpp)
...
| `tools/pm_probe.cpp` | Wizard output format; layout mode detection; scan root candidate detection |
...
| `tools/pm_probe.cpp` SN | Bump to next HWM |
...
7. Update `pm_probe` wizard to emit new format
