...
SettingsDialog::SettingsDialog(QWidget* parent)
    : QDialog(parent)
{
    setWindowTitle("CamClops Settings");
...
void SettingsDialog::buildGeneralTab(QTabWidget* tabs)
{
    ...
    m_jobQueueMode = new QComboBox;
    m_jobQueueMode->addItem("Docked panel (bottom of window)", "docked");
    ...
    m_monitorPort->setToolTip("HTTP port for the remote job monitor (clops_monitor.py). "
                              "Takes effect next time the monitor is started.");
    form->addRow("Monitor port:", m_monitorPort);
    ...
    form->addRow("", new QLabel("<i>Profile ID (stem of ~/.config/camclops/profiles/<id>.json)</i>"));
...
void SettingsDialog::buildEncoderTab(QTabWidget* tabs)
{
    ...
    auto* note = new QLabel(
        ...
        "(camclops_&lt;hostname&gt;.json) and override the base config on this machine.</small>");
...
void SettingsDialog::buildHostTab(QTabWidget* tabs)
{
    ...
    m_hostTmpDir = new QLineEdit; m_hostTmpDir->setPlaceholderText("(auto: <export dir>/clops_tmp)");
...
void SettingsDialog::buildHudTab(QTabWidget* tabs)
{
    auto* w    = new QWidget;
    auto* form = new QFormLayout(w);
    form->setContentsMargins(12, 12, 12, 12);
    ...
    form->addRow(new QLabel(
        "<i>These settings apply to all HUD overlays generated by clops_hud.py.</i>"));

    m_hudFontScale = new QDoubleSpinBox;
    m_hudFontScale->setRange(0.25, 4.0);
    m_hudFontScale->setSingleStep(0.25);
    m_hudFontScale->setDecimals(2);
    m_hudFontScale->setSuffix("×");
    m_hudFontScale->setToolTip("Multiplier applied to all HUD font sizes (1.0 = default)");
    form->addRow("Font scale:", m_hudFontScale);

    m_hudLineScale = new QDoubleSpinBox;
    m_hudLineScale->setRange(0.25, 4.0);
    m_hudLineScale->setSingleStep(0.25);
    m_hudLineScale->setDecimals(2);
    m_hudLineScale->setSuffix("×");
    m_hudLineScale->setToolTip("Multiplier applied to all HUD line and tick mark widths (1.0 = default)");
    form->addRow("Line scale:", m_hudLineScale);

    m_hudColor = new QLineEdit;
    m_hudColor->setMaxLength(7);
    m_hudColor->setPlaceholderText("#00ff41");
    m_hudColor->setToolTip("Element color as a CSS hex value — e.g. #00ff41 (phosphor green), #ffaa00 (amber)");
    form->addRow("Element color:", m_hudColor);

    tabs->addTab(scrollWrap(w), "HUD");
}
...
void SettingsDialog::loadFromConfig()
{
    ...
    {
        QString mode = QString::fromStdString(s.jobQueueMode);
        int idx = 0;
        for (int i = 0; i < m_jobQueueMode->count(); ++i) {
            if (m_jobQueueMode->itemData(i).toString() == mode) { idx = i; break; }
        }
    ...
    // HUD
    m_hudFontScale->setValue(s.hudFontScale);
    m_hudLineScale->setValue(s.hudLineScale);
    m_hudColor->setText(QString::fromStdString(s.hudColor));
...
void SettingsDialog::saveToConfig()
{
    ...
    // HUD
    s.hudFontScale = m_hudFontScale->value();
    s.hudLineScale = m_hudLineScale->value();
    {
        QString c = m_hudColor->text().trimmed();
        s.hudColor = (!c.isEmpty() && c.startsWith('#')) ? c.toStdString() : "#00ff41";
    }

    m_config.applySettings(s);
    m_config.saveSettings();      // writes camclops.json
    m_config.saveHostSettings();  // writes camclops_<hostname>.json
