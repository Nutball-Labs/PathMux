...
// overlay scripts (pm_maprender.py, pm_dashboard.py, pm_hud.py) in sequence.
...
//
// Usage:
//   pm_videos --mid=XX [--mid=YY ...] --all
//   pm_videos --tid=MM:TT [--tid=MM:UU ...] --map --dash
//   pm_videos --mid=XX --force --dry-run
...
static std::string findScript(const std::string& argv0, const std::string& scriptName) {
    ...
    const std::vector<fs::path> candidates = {
        ...
        binDir / ".." / "share" / "pathmux" / "scripts" / scriptName, // Linux installed
...
int main(int argc, char* argv[]) {
    ...
    if (doMap)  mapScript  = findScript(argv[0], "pm_maprender.py");
    if (doDash) dashScript = findScript(argv[0], "pm_dashboard.py");
    if (doHud)  hudScript  = findScript(argv[0], "pm_hud.py");

    if (doMap  && mapScript.empty())  { std::cerr << "Error: pm_maprender.py not found\n"; return 1; }
    if (doDash && dashScript.empty()) { std::cerr << "Error: pm_dashboard.py not found\n"; return 1; }
    if (doHud  && hudScript.empty())  { std::cerr << "Error: pm_hud.py not found\n";       return 1; }
    ...
    for (const auto& qe : queue) {
        ...
        if (tp->gpsTrackStatus != "complete") {
            ...
            if (!dryRun) {
                ...
                int seg = 0;
                bool gpsOk = Pathmux::extractGps(root, tripIdx, qe.manifestFile,
        ...
        if (doMap) {
            ...
            ok &= buildVideo("map",
                mapScript,
                srcDir + "/pm_trip_" + tp->id + "_map.mp4",
        ...
        if (doDash) {
            ...
            ok &= buildVideo("dash",
                dashScript,
                srcDir + "/pm_trip_" + tp->id + "_dash.mp4",
                960, 540,
                extra, tp->dashVideos);
        }
        ...
        if (doHud) {
            ...
            ok &= buildVideo("hud",
                hudScript,
                srcDir + "/pm_trip_" + tp->id + "_hud.webm",
                HUD_W, HUD_H,
                extra, tp->hudVideos);
        }
