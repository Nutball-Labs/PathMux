...
//
// Usage:
//   clops_gpsexport MID:TID --gpx  [path]
//   clops_gpsexport MID:TID --kml  [path]
//   clops_gpsexport MID:TID --geojson [path]
//   clops_gpsexport MID:TID --dump                         (dump track to terminal)
//   clops_gpsexport MID:TID --gpx out.gpx --kml out.kml    (multiple formats)
//   clops_gpsexport MID:TID --gpx /dir/    (auto-name: YYYYMMDD_HHMMSS.gpx)
//   clops_gpsexport MID:TID --force        (overwrite existing files)
//   clops_gpsexport MID:TID --verbose      (show exiftool stderr during extraction)
//
// MID:TID addresses a trip by manifest ID and trip ID (e.g. F0:4P).
// At least one output flag (--gpx, --kml, --geojson, --dump) is required.
// If GPS has not yet been extracted, clops_gpsexport runs ExifTool extraction
...
static std::string buildStem(const json& jTrip)
{
    ...
    if (front.empty()) return "clops_trip";
    ...
    if (!fname.empty() && std::isalpha(static_cast<unsigned char>(fname.back())))
        fname.pop_back();
    return fname.empty() ? "clops_trip" : fname;
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help")     { printUsage(argv[0]); return 0; }
            ...
            std::cout << APP_NAME << " clops_gpsexport v" << APP_VERSION << "\n"; return 0;
        }
    ...
    if (needsGps) {
        ...
        if (!CamClops::extractGps(root, tripIdx, manifestFile, etPath, verbose)) {
    ...
    if (doGpx) {
        std::string out = CamClops::writeGpx(root, tripIdx, resolvedGpx);
        if (out.empty()) { exitCode = 1; }
        else { std::cout << out << "\n"; }
    }
    if (doKml) {
        std::string out = CamClops::writeKml(root, tripIdx, resolvedKml);
        if (out.empty()) { exitCode = 1; }
        else { std::cout << out << "\n"; }
    }
    if (doGeoJson) {
        std::string out = CamClops::writeGeoJson(root, tripIdx, resolvedGeoJson);
        if (out.empty()) { exitCode = 1; }
        else { std::cout << out << "\n"; }
    }
    if (doDump) {
