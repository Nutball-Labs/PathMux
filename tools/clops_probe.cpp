...
// clops_probe — camera compatibility profiler (CamClops suite)
//
// Usage:
//   clops_probe <file.ts>               Probe a single segment
//   clops_probe MID:TID                 Probe first Front segment of a known trip
//   clops_probe --card <path>           Fingerprint a full dashcam storage root
//   clops_probe --card <path> --json    Machine-readable fingerprint
//   clops_probe --json <file.ts>        Machine-readable single-file probe
//
...
static void printFileProbe(const FileProbe& p)
{
    ...
    std::cout << "File:         " << p.filePath << "\n"
              ...
              << "Frame rate:   " << CamClops::formatFrameRate(p.video.frameRate) << "\n"
                       ...
                       const std::string& exiftoolOptions)
{
    ...
    if (jsonMode) {
        ...
        j["submit_to"] = "https://github.com/Nutball-Labs/CamClops/issues";
    ...
    // ---- Text output ----
    std::cout << "--- clops_probe SD Card Fingerprint ---\n"
              << "Root:         " << scanRoot << "\n";
    ...
    std::cout << "Video profile (" << fs::path(primaryDir).filename().string()
              ...
              << "  Frame rate:   " << CamClops::formatFrameRate(probe.video.frameRate) << "\n"
    ...
    std::cout << "\n--- Submit this report to: "
              << "https://github.com/Nutball-Labs/CamClops/issues ---\n";
}
                        ...
                        const std::string& exiftoolOptions)
{
    ...
    std::cout << "--- clops_probe: Camera Profile Wizard ---\n"
              << "Scanning: " << root << "\n";
    std::cout.flush();
    ...
    for (const auto& c : cams) {
        ...
        if (c.probe.video.width > 0)
            std::cout << "    video: " << c.probe.video.width << "x"
                      ...
                      << CamClops::formatFrameRate(c.probe.video.frameRate)
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") { printUsage(argv[0]); return 0; }
            ...
            std::cout << APP_NAME << " clops_probe v" << APP_VERSION << "\n"; return 0;
        }
    ...
    if (cardMode) {
        if (cardPath.empty()) {
            // Allow: clops_probe --card /path  OR  clops_probe /path --card
            cardPath = targetArg;
        }
        if (cardPath.empty()) {
