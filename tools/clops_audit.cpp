...
// clops_audit — footage integrity checker (CamClops suite)
//
// Usage:
//   clops_audit                 Audit all manifests
//   clops_audit MID             Audit one manifest
//   clops_audit MID:TID         Audit one trip
//   clops_audit --deep          Also run ffprobe on every segment
//   clops_audit --json          Machine-readable output
//
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") { printUsage(argv[0]); return 0; }
            ...
            std::cout << APP_NAME << " clops_audit v" << APP_VERSION << "\n"; return 0;
        }
