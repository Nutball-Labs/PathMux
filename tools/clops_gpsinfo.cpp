...
// clops_gpsinfo — standalone GPS inspection tool for CamClops suite.
// Runs exiftool on a Pruveeo D90 .ts file and reports GPS data.
//
// Usage:
//   clops_gpsinfo [options] <file.ts>
//   clops_gpsinfo --scan-all-trips [options]
//
// Options:
...
// ---------------------------------------------------------------------------
// Epoch helpers — mirrors clops_findgpslock logic
// ---------------------------------------------------------------------------
...
static void printUsage(const char* argv0) {
    (void)argv0;
    std::cerr
        << "Usage: clops_gpsinfo [options] <file.ts | MID:TID>\n"
        << "       clops_gpsinfo --scan-all-trips [options]\n\n"
        << "Single-file options:\n"
        ...
        << "  --scan-all-trips reads manifests from ~/.config/camclops/\n";
}
    ...
    const std::string& exiftoolOpts)
{
    ...
    if (index.empty()) {
        std::cerr << "Error: No manifests found in "
                  ...
                  << "  Has camclops been run at least once to build manifests?\n";
        return results;
    }
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") { printUsage(argv[0]); return 0; }
            ...
            std::cout << APP_NAME << " clops_gpsinfo v" << APP_VERSION << "\n"; return 0;
        }
